...
- **OAuth PKCE with pop-out sign-in** — Authorization Code + PKCE, no client secret needed. Genesys is retiring iframe-embedded login ([announcement](https://help.genesys.cloud/announcements/genesys-cloud/deprecation-ability-to-embed-the-genesys-cloud-login-web-application-within-an-iframe/); new integrations 2026-08-31, all integrations 2027-02-04), so the app never navigates its own frame to the login page. A **Sign in** button opens a same-origin pop-out window that runs the whole flow top-level and returns the token via `postMessage`. The `redirect_uri` is unchanged, so no OAuth client reconfiguration is needed — but pop-ups must be allowed for the app origin. The access token lives in `sessionStorage` only, scoped to the tab that obtained it.
